...
\end{itemize}
\end{teo}
\subparagraph*{Criterio del rapporto}
\begin{teo}
Sia  $\sum_{n=1}^{\infty}a_n$ una serie numerica definitivamente positiva. Esista $\alpha$ tale che
...
Un modo più semplificato per risolvere queste equazioni è quello di supporre che le soluzioni siano proporzionali a $e^{\lambda_i t}$.
Sostituendo questa funzione nell'equazione originale si possono determinare i vari $\lambda_i$.
In altre parole: sia $\lambda \in \mathbb{C}$: la funzione $e^{\lambda t}$ è soluzione di (ELOCC) se e solo se $\lambda$ è uno zero del polinomio caratteristico di (ELOCC) definito da: $$ p(\lambda) := \lambda^n + a_1\lambda^{(n-1)}+ \dots +a_{n-1}\lambda+a_n$$
A questo punto la soluzione è sono del tipo:
$$\phi(t) = \sum_{i=1}^n c_ie^{\lambda_it}$$
con ovvie costanti $c_i$ eventualmente da determinare.
